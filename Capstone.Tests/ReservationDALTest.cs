...
namespace Capstone.Tests
{
    ...
    public class ReservationDALTest
    {
        ...
        //Test Reservation Member Variables
        private int _reservationId;
        private const string TestReservationName = "Test Family Reservation";
        private DateTime TestReservationFrom = new DateTime(2018-09-06);
        private DateTime TestReservationTo = new DateTime(2018-09-26);
        private DateTime TestReservationCreate = DateTime.Now;
        ...
        public void Initialize()
        {
            ...
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                cmd = new SqlCommand("INSERT INTO reservation (site_id, name, from_date, to_date, create_date) " +
                                    $"VALUES ('{_siteId}', '{TestReservationName}', '{TestReservationFrom}', '{TestReservationTo}', '{TestReservationCreate}'); " +
                                     "SELECT CAST(SCOPE_IDENTITY() as int);", conn);

                _reservationId = (int)cmd.ExecuteScalar();
        ...
        {
            //Arrange 
            ReservationDAL reservationDAL = new ReservationDAL(_connectionString);

            //Act
            List<Reservation> reservations = reservationDAL.GetCurrentReservations(site);

            //Assert
            Assert.AreEqual(1, reservations.Count);
            Assert.AreEqual(_reservationId, reservations[0].ReservationId);
        }
    }
}
